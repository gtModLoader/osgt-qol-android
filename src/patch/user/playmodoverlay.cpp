...
#include "game/struct/inventorymenuinfo.hpp"
#include "game/struct/variant.hpp"

#include "game/struct/custom/playmodoverlay.hpp"
#include "game/struct/net/enet.hpp"

REGISTER_GAME_FUNCTION(GetMessageTypeFromPacket, "_Z24GetMessageTypeFromPacketP11_ENetPacket", int, void*);
REGISTER_GAME_FUNCTION(OnLogGrabBarChanged, "_Z19OnLogGrabBarChangedP7Variant", void, Variant*);
REGISTER_GAME_FUNCTION(GameLogicComponentKillWorld, "_ZN18GameLogicComponent9KillWorldEb", void, GameLogicComponent*,
                       bool);
REGISTER_GAME_GLOBAL_VAR(g_inventoryInfo, "g_inventoryInfo", InventoryMenuInfo*);
...
static float g_lastOverlayX = 16.0f;
...
class PlaymodTimersOverlay : public patch::BasePatch
{
  public:
    void apply() const override
    {
        ...
        // instead.
        game.hookFunctionPattern<GetMessageTypeFromPacket_t>(pattern::GetMessageTypeFromPacket,
                                                             GetMessageTypeFromPacket, &real::GetMessageTypeFromPacket);

        // We anchor into OnLogGrabBarChanged so we can position ourselves against it.
        game.hookFunctionPattern<OnLogGrabBarChanged_t>(pattern::OnLogGrabBarChanged, OnLogGrabBarChanged,
                                                        &real::OnLogGrabBarChanged);

        // We should cleanup when player leaves world as well, so we need to know an event for it.
        game.hookFunctionPattern<GameLogicComponentKillWorld_t>(
            pattern::GameLogicComponentKillWorld, GameLogicComponentKillWorld, &real::GameLogicComponentKillWorld);

        real::g_inventoryInfo = (InventoryMenuInfo*)dlsym(game.getGameHandle(), pattern::g_inventoryInfo.c_str());
    ...
    static void GameLogicComponentKillWorld(GameLogicComponent* this_, bool bUnk)
    {
        // Clear any old ui
        Entity* pWorldSpecificGUI =
            real::GetApp()->m_pEntityRoot->GetEntityByName("GUI")->GetEntityByName("WorldSpecificGUI");
    ...
    static void OnLogGrabBarChanged(Variant* pV)
    {
        ...
        if (!pLogEnt)
        {
            ...
            xCoord = pLogEnt->GetVar("pos2d")->GetVector2().x + 16;
        }
        else
        {
        ...
        if (!pOverlayEnt)
            return;
        pOverlayEnt->GetVar("pos2d")->Set(CL_Vec2f(xCoord, yLevel));
        g_lastOverlayHeight = yLevel;
        g_lastOverlayX = xCoord;
    }

    static int GetMessageTypeFromPacket(void* _packet)
    {
        ...
        case PLAYMOD_PROTOCOL_ADD_OR_REMOVE:
        {
            ...
            {
                LOG_DEBUG("[PlaymodOverlays] invaid add/del mod payload.\n");
                break;
            }
            int offset = 10;
            ...
            for (auto it = g_activeMods.begin(); it != g_activeMods.end();)
            {
                if (it->m_playmodID == modData->PlaymodID)
                {
                    ...
                    if (!DoesTimerOverlayExist())
                    {
                        ...
                        {
                            LOG_DEBUG("[PlaymodOverlays] killing offscreen\n");
                    ...
                    LOG_DEBUG("[PlaymodOverlays] upd kstamp=%ld, oldstamp=%ld, dur=%d, pmod=%d\n", it->m_killstamp,
            ...
            {
                LOG_DEBUG("[PlaymodOverlays] new pmod target\n");
                ActiveMod mod;
        ...
        case PLAYMOD_PROTOCOL_SEND_BULK:
        {
            ...
            {
                LOG_DEBUG("[PlaymodOverlays] invalid bulk data payload.\n");
                break;
            }
            int offset = 10;
        ...
        case PLAYMOD_PROTOCOL_UPDATE_ICON_DATA:
        {
            ...
            {
                LOG_DEBUG("[PlaymodOverlays] invalid icon data payload.\n");
                break;
            }
    ...
    static void ConstructOverlay()
    {
        ...
        pOverlayEnt->GetVar("pos2d")->Set(CL_Vec2f(g_lastOverlayX, g_lastOverlayHeight));
    ...
    static PlaymodTimerEntity* CreateTicker(int ItemID, Entity* ParentEnt, CL_Vec2f& bounds)
    {
        ...
        CL_Vec2f IconBounds(8.f * (real::g_inventoryInfo->m_scale / 1.33f),
                            8.f * (real::g_inventoryInfo->m_scale / 1.33f));
        Entity* pIcon = DrawIcon(ItemID, pTimerEnt, IconBounds);
        Entity* pTextLabel =
            real::CreateTextLabelEntity(pTimerEnt, "txt", 32.0f * (real::g_inventoryInfo->m_scale / 1.33f),
                                        4.f * (real::g_inventoryInfo->m_scale / 1.33f), "0:00");
        SetTextShadowColor(pTextLabel, 150);
        pTextLabel->GetVar("scale2d")->Set((real::g_inventoryInfo->m_scale / 1.33f) / 2.0f);
        ...
        if (g_playmodIconHeight == 0.0f || g_bRecalculateIconHeight)
        {
            ...
            g_playmodIconHeight = pIcon->GetVar("size2d")->GetVector2().y * (real::g_inventoryInfo->m_scale / 1.33f);
    ...
    static Entity* DrawIcon(int ItemID, Entity* ParentEnt, CL_Vec2f& bounds)
    {
        ...
        pIcon->GetVar("scale2d")->Set(CL_Vec2f((real::g_inventoryInfo->m_scale / 1.33f)));
